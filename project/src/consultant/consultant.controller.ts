...
export class ConsultantController {
  ...
  async addClientToProject(@Param('idCons') idCons: number, @Param('idProj') idProj: number) {
    return await this.consultantService.addConsultantToProject(idCons, idProj);
  }

  @Delete(':idCons/projects/:idProj')
  async removeClientFromProject(@Param('idCons') idCons: number, @Param('idProj') idProj: number) {
